...
public class ClickHouseUpsertExecutor implements ClickHouseExecutor {
    ...
    public void closeStatement() throws SQLException {
        ...
        List<PreparedStatement> clickHousePreparedStatements = Arrays.asList(this.insertStmt, this.updateStmt, this.deleteStmt);
        for (PreparedStatement stmt : clickHousePreparedStatements) {
            if (stmt != null) {
                stmt.close();
            }
        }
            ...
            for (int i = 0; i < ClickHouseUpsertExecutor.this.maxRetries; i++) {
                try {
                    ...
                    try {
                        ...
                        throw new IOException("unable to flush; interrupted while doing another attempt", exception);
                    }
                }
            }
        }
